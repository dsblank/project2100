...
def play_measure(instrument, percussion, year, angle, tempo):
    ...
    play_note(instrument, percussion, tfreq1, 0.5, 
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if scan is None:
            break
        data = convert_depth(scan) # 0 is close, 255 is far away
        rows, cols = data.shape
        maximum = 0
        maximum_col = 0
        minimum = 255
        minimum_col = 0
        pic = np.zeros(shape=(int(rows * .33), cols), dtype="uint8")
        r = 0
        for row in range(int(rows * .33), int(rows * .66)): # 480 middle third
            for col in range(cols): # 640
                ...
                pic[r][col] = data[row][col]
            r += 1
        angle = minimum_col/640.0
        print("min angle:", angle, "distance:", minimum)
        time.sleep(2)
        tempo = (year - 1900)/230.0
        year = minimum/255.0 * 200.0 + 1900
        play_measure(instr1, percussion, year, angle, tempo)
